...
- **UI: a dedicated screen, not a menu.** `home_screen.dart` keeps Sign out as a plain, always-visible AppBar icon (a reversible action — no extra friction needed) but pulls Delete Account out into its own small `TextButton.icon` pinned to the bottom-right corner via a `Stack`/`Positioned` (deliberately off to the side of Play/Daily Challenge/Leaderboard, never among them). Tapping it pushes `lib/screens/delete_account_screen.dart`, a full screen that states plainly what gets deleted (account/sign-in, display name, score, streak, Daily Challenge history/leaderboard placement) before its one destructive "Delete My Account" button — reaching the screen at all, plus deliberately pressing that button, *is* the confirmation step, so there's no dialog stacked on top of it. A "Cancel" button returns to Home without side effects. On success, `Navigator.of(context).popUntil((route) => route.isFirst)` — needed because this screen was *pushed* on top of Home, and `main.dart`'s root `_AuthGate` only controls what sits at the *bottom* of the navigation stack; without the explicit pop, the auth-gate's swap to the login flow would happen invisibly underneath this still-stacked screen (same pattern `results_screen.dart`'s Home button already uses). A failed deletion surfaces its error inline on the screen and re-enables the button, never crashes or navigates away.
- **Deployed and live-verified** — `delete-own-account` is deployed (`supabase functions deploy delete-own-account`) and confirmed `ACTIVE` on the project.

**Web half ✅ built** — a static instructions page (Google's policy accepts either a live delete form or clear instructions; the latter is what this is), published as a Claude Artifact by your choice over GitHub Pages (your main repo is private, which rules out plain Pages there without a paid plan): https://claude.ai/code/artifact/cb260251-5646-4f4d-bafb-edf93005482e — lists exactly what's stored (account, display name, score, streak, Daily Challenge/leaderboard history) and both deletion paths (in-app self-service, immediate; email request, 14 days). **Must be set to public via the page's share menu before it's usable for Play Console** — it publishes private by default. Contact email defaults to your own (`[SUPPORT_EMAIL]`) — swap it for a dedicated support address if/when you have one.
...
## 6. Phase 4 — Ads Integration ✅ Built (banner, interstitial, and rewarded)

Rewarded ads were originally dropped from this phase — the spec's "revive/double-coins" mechanic no longer exists post the marks-scoring redesign. Revisited once a real reward existed to attach it to: **"Watch Ad for Hint"** replaced the plain "Show hint" button at tier 3-4 (`game_screen.dart`'s `_buildHintButton`). Watching to completion (`RewardedAd`'s `onUserEarnedReward`, never for skipping/closing early) reveals the hint; if no ad happens to be ready (load in flight, network hiccup, consent not yet resolved), the hint still reveals for free — the same "best-effort, never load-bearing" rule every other ad in this codebase follows, since a hint is a nice-to-have, not worth stranding a player over.
...
- **`lib/services/ads_service.dart`** (new) — the actual `google_mobile_ads` + UMP wrapper (singleton `AdsService.instance`, since it holds mutable ad state unlike this codebase's other static services). Every public method is defensive (try/catch, `kIsWeb`-guarded) — `google_mobile_ads` has no Flutter Web support at all and its platform channel isn't registered under `flutter test` either, so an ad SDK failure must never break app startup or break an existing widget test, the same "best-effort" precedent already set for every Supabase call. Uses Google's published **TEST** ad-unit IDs for banner, interstitial, *and* rewarded (need no AdMob account) — must be swapped for real ones before any release build.
- **Placements**: banner shows on `home_screen.dart`'s `bottomNavigationBar` **and** on `game_screen.dart`, top of screen, right after the marks/timer bar and before the question (by explicit product decision, superseding this section's original "Home only" call — never mid-question-content or overlapping the options). Interstitial triggers from `game_screen.dart`'s `_GameScreenState` — detects a completed question via a `questionNumber` transition in the existing controller-change listener, records it on `AdFrequencyCap`, and shows via `AdsService` only between questions (never mid-countdown) and only when the session isn't also ending on that same transition (so it never stacks in front of the results-screen navigation).
...
**Manual test checklist** (needs a real Android emulator/iOS simulator — this dev loop has been Chrome-only so far, and `google_mobile_ads` has no web support at all):
  - [ ] Banner shows on Home and on the game screen (top, after the marks/timer bar), never overlapping the question/options/countdown.
  ...
  - [ ] "Watch Ad for Hint" at tier 3-4 plays a rewarded ad and reveals the hint only after watching to completion; closing/skipping the ad early does not reveal it (but a missing/unloaded ad still falls back to free, by design).
...
- [x] Account deletion, in-app half: `delete-own-account` Edge Function + `AuthService.deleteAccount` + Home's corner button → dedicated `DeleteAccountScreen` (see §4c) — 339 tests passing
